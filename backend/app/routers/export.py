...
_EXPORT_DISCLAIMER = (
    "DISCLAIMER: This export is generated by Echelon, an open-source GEOINT research tool. "
    "Data is aggregated from multiple open sources with varying licenses, accuracy, and timeliness. "
    "Convergence scores are statistical indicators, not confirmed events. "
    "AI copilot outputs (if included) may contain errors or hallucinations. "
    "This data is NOT a substitute for professional intelligence analysis. "
    "Users are responsible for verifying all information independently and complying "
    "with source-specific attribution and licensing requirements. "
    "See https://github.com/chrislyonsKY/echelon/blob/main/docs/source-licensing-matrix.md "
    "for per-source license details."
)

_ATTRIBUTION = {
    "gdelt": "GDELT Project (gdeltproject.org) — open access",
    "gfw": "Global Fishing Watch (globalfishingwatch.org) — CC BY-SA 4.0",
    "opensky": "OpenSky Network (opensky-network.org) — ODbL",
    "osm": "OpenStreetMap contributors (openstreetmap.org) — ODbL",
    "sentinel2": "Copernicus Sentinel-2 via Element84 — open access",
    "firms": "NASA FIRMS — US Government public domain",
    "newsdata": "NewsData.io — API terms apply",
    "osint_scrape": "Various RSS/OSINT sources — see individual source attribution",
    "aisstream": "AISStream.io — API terms apply",
    "acled": "ACLED (acleddata.com) — restricted academic license",
}
...
async def export_geojson(
    ...
    geojson = {
        ...
        "metadata": {
            ...
            "count": len(features),
            "generator": "Echelon GEOINT Platform v0.2.0",
            "disclaimer": _EXPORT_DISCLAIMER,
            "sourcesIncluded": sources_used,
            "attribution": {s: _ATTRIBUTION.get(s, "See source documentation") for s in sources_used},
        },
    }
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    ...
    <description>{_xml_escape(f"Exported {len(placemarks)} signals, {days}-day window. " + _EXPORT_DISCLAIMER)}</description>
...
async def export_csv(
    ...
    writer.writerow([f"# {_EXPORT_DISCLAIMER}"])
    writer.writerow([
